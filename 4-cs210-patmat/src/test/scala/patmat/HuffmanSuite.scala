...
class HuffmanSuite {
  ...
  @Test def `times working trivial`: Unit =
    val tested = List[Char]('a', 'a', 'a', 'b', 'b', 'c', 'z', 'z', 'z', 'z')
    val expected = List[(Char, Int)](('a', 3), ('b', 2), ('c', 1), ('z', 4))
    assertEquals(expected, times(tested))
