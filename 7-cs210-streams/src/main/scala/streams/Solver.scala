...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = 
    val set = 
      for {
        neighbors <- b.legalNeighbors // List[(Block, Move)]
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = 
    if initial.isEmpty then List.empty.to(LazyList)
    else 
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = 
    pathsFromStart.filter(lazylist => done(lazylist._1)) 
  ...
  lazy val solution: List[Move] = 
    ...
    paths match 
      case Nil => Nil
      case _ => paths.map(lazylist => lazylist._2).head.reverse 
}
