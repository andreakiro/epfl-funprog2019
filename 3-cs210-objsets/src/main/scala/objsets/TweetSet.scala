...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted = 
    def maxRetweet(twt1: Tweet, twt2: Tweet): Tweet =
      if twt1.retweets > twt2.retweets then twt1
      else twt2
    if left.isEmpty && right.isEmpty then elem
    else if left.isEmpty then maxRetweet(elem, right.mostRetweeted)
    else if right.isEmpty then maxRetweet(elem, left.mostRetweeted)
