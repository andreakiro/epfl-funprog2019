...
object RecursiveLanguage {
    ...
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => 
        eval(scrutinee, defs) match
          ...
          case Cons(head, tail) => eval(caseCons, defs.updated(headName, head).updated(tailName, tail))
          case _ => error("this is a magic error")
    ...
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        if headName == n || tailName == n then
          ...
          if free.contains(headName) || free.contains(tailName) then 
            val h1 = differentName(headName, free)
            val t1 = differentName(tailName, free)
            val cc1 = alphaConvert(alphaConvert(caseCons, headName, h1), tailName, t1)
            Match(subst(scrutinee, n, r), subst(caseEmpty, n, r), h1, t1, subst(cc1, n, r))
  ...
  def freeVars(e: Expr): Set[String] =
    e match
      ...
      // TODO: Add cases for Empty, Cons & Match DONE
      case Empty => Set()
      case Cons(head, tail) => freeVars(head) ++ freeVars(tail)
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => 
          scrutinee match
            case Empty => freeVars(caseEmpty)
            case Cons(head, tail) => freeVars(caseCons) + headName + tailName
            case _ => error("this is a magic error")
  ...
  def alphaConvert(e: Expr, n: String, m: String): Expr =
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        if n == headName || n == tailName then e
          //Match(alphaConvert(scrutinee, n, m), alphaConvert(caseEmpty, n, m), headName, tailName, caseCons)
        else 
          Match(scrutinee, alphaConvert(caseEmpty, n, m), headName, tailName, alphaConvert(caseCons, n, m))
          //Match(alphaConvert(scrutinee, n, m), alphaConvert(caseEmpty, n, m), headName, tailName, alphaConvert(caseCons, n, m))
  ...
  def show(e: Expr): String =
    e match
      ...
      case Empty => null
      case Cons(head, tail) => null
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => null
  ...
  val definitions: DefEnv = Map[String, Expr](
    ...
    "map" -> Fun("ls", 
                  Fun("f", 
                      Match(Name("ls"), Empty, "x", "xs", 
                              Cons(Call(Name("f"), Name("x")), Call(Call(Name("map"), Name("xs")), Name("f")))))),

    // TODO Implement gcd (see recitation session)
    "gcd" -> Fun("x", Fun("y", IfNonzero(Name("y"),
                  Call(Call(Name("gcd"), Name("y")), modulo(Name("x"), Name("y"))), Name("x")))),

    // TODO Implement foldLeft (see recitation session)
    "foldLeft" -> Fun("ls", Fun("acc", Fun("fold", 
                    Match(Name("ls"), Name("acc"), "x", "xs", 
                    Call(Call(Call(Name("foldLeft"), Name("xs")), Call(Call(Name("fold"), Name("acc")), Name("x"))), Name("fold")))))),

    // TODO Implement foldRight (analogous to foldLeft, but operate right-to-left)
    "foldRight" -> Fun("ls", Fun("acc", Fun("fold", 
                    Match(Name("ls"), Name("acc"), "x", "xs", 
                    Call(Call(Name("fold"), Name("x")), Call(Call(Call(Name("foldRight"), Name("xs")), Name("acc")), Name("fold"))))))),
  )
