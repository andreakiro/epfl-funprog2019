...
trait GivenDecoders {
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction {
      case Json.Num(v) if v.isValidInt => v.toInt
    }
  ...
  given [A](given decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction {
      case Json.Arr(items) => items.map(i => i.decodeAs[A].get)
    }
