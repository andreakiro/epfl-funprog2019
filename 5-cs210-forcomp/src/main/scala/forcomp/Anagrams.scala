...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = 
    w.toLowerCase.toSeq.groupBy((c: Char) => c).view.mapValues(_.size).toList.sortBy(_._1)
    ...
    occurrences match
      ...
      case h :: t =>
        (for {
          freq <- 0 to h._2
          sub <- combinations(t)
        } yield if (freq == 0) sub else (h._1, freq) :: sub) toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = 
    y match
      case Nil => x
      case h :: t => 
        val temp = x.map((tp: (Char, Int)) => (tp._1, tp._2 - (y.toMap.getOrElse(tp._1, 0))))
        temp.filter((tp: (Char, Int)) => tp._2 != 0).sorted  
      ...
      occurrences match 
        ...
        case h :: t =>
          for {
            ...
            word <- dictionaryByOccurrences.withDefaultValue(List())(subset)
            subanagrams <- subanagrams(subtract(occurrences, subset))
          } yield word :: subanagrams
